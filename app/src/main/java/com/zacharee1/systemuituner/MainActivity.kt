...
class MainActivity : AppCompatActivity(), NavController.OnDestinationChangedListener {
    ...
    private fun setUpDrawer() {
        with(mainBinding.slider) {
            customWidth = dpAsPx(250)
            headerPadding = true
            headerHeight = DimenHolder.fromDp(172)
            headerView = LayoutInflater.from(this@MainActivity).inflate(R.layout.drawer_header, null)
            setupWithNavController(navController)
            recyclerView.setBackgroundColor(ContextCompat.getColor(this@MainActivity, R.color.toolbarColor))

            val currentListener = onDrawerItemClickListener
